...
class SqlParserFactoryTest {
    ...
    void assertCreateParserForMySql() {
        assertDoesNotThrow(() -> SqlParserFactory.createParser("SELECT 1", "MYSQL"));
    }

    @Test
    void assertCreateParserForPostgreSql() {
        assertDoesNotThrow(() -> SqlParserFactory.createParser("SELECT 1", "POSTGRESQL"));
    }

    @Test
    void assertCreateParserForOracle() {
        assertDoesNotThrow(() -> SqlParserFactory.createParser("SELECT 1 FROM DUAL", "ORACLE"));
    }

    @Test
    void assertCreateParserForSqlServer() {
        assertDoesNotThrow(() -> SqlParserFactory.createParser("SELECT 1", "SQLSERVER"));
    }
    ...
    void assertParseWithOffsetFetch() throws SqlParseException {
        ...
        SqlNode result = SqlParserFactory.parse(sql, "POSTGRESQL");
        assertNotNull(result);
    }
    ...
    void assertParseInvalidSqlThrowsException() {
        ...
        assertThrows(SqlParseException.class, () -> SqlParserFactory.parse(sql, "MYSQL"));
    }

    @Test
    void assertParseExpression() throws SqlParseException {
        String expr = "1 + 2 * 3";
        SqlNode result = SqlParserFactory.parseExpression(expr, "MYSQL");
        assertNotNull(result);
    }
