...
package org.intellisql.connector.mysql;
...
public class MySQLConnector implements DataSourceConnector {
    ...
    private final Map<String, MySQLConnectionPool> connectionPools = new ConcurrentHashMap<>();
    ...
    public Connection connect(final DataSourceConfig config) throws Exception {
        ...
        return new MySQLConnection(jdbcConnection, pool);
    }
    ...
    public boolean testConnection(final DataSourceConfig config) {
        try {
            ...
            log.info(
                    "MySQL connection test for '{}': {}", config.getName(), success ? "SUCCESS" : "FAILED");
            return success;
            // CHECKSTYLE:OFF: IllegalCatch
        } catch (final Exception ex) {
            // CHECKSTYLE:ON: IllegalCatch
            log.error("MySQL connection test failed for '{}': {}", config.getName(), ex.getMessage(), ex);
            return false;
        }
    }
    ...
    private MySQLConnectionPool getOrCreatePool(final DataSourceConfig config) {
        return connectionPools.computeIfAbsent(
                config.getName(),
                name -> {
                    log.info("Creating new MySQL connection pool for: {}", name);
    ...
    @Override
    public void close() {
        log.info("Closing all MySQL connection pools");
        connectionPools.values().forEach(MySQLConnectionPool::close);
    ...
    public void closePool(final String name) {
        MySQLConnectionPool pool = connectionPools.remove(name);
        if (pool != null) {
            pool.close();
            log.info("Closed MySQL connection pool for: {}", name);
        }
    }
    ...
    public int getActiveConnections(final String name) {
        MySQLConnectionPool pool = connectionPools.get(name);
